    ...
    messages=[
        # 1️⃣  Hard contract
        {
            ...
            "content": (
                ...
                "3. **Paragraphs:** Preserve the paragraph structure from the handwritten input. Use blank lines in the LaTeX source to separate paragraphs.\n"
                "4. **Chapter Titles:** Identify text formatted like chapter or section titles (e.g., centered, larger font, underlined). Format these using the `\\section*{Title Text}` command. Do not automatically number sections.\n"
                "5. **Ignore:** Do NOT transcribe any dates or times found in the document.\n"
                "6. **Chemical Structures/Reactions:** For handwritten molecular drawings or reaction schemes that are part of an equation or text flow:\n"
                ...
                "7. **General Diagrams:** For graphs, plots, illustrations, flowcharts, etc. NOT inline:\n"
                "   - On a line *by itself*, write exactly `% PLACEHOLDER: DIAGRAM-M` (start M at 1).\n"
                "   - Do NOT insert the raw label (DIAGRAM-M) directly.\n"
                "8. Numbering: Use separate counters for STRUCTURE-N and DIAGRAM-M (within the placeholders and descriptions). Renumber rules as needed.\n"
                "9. Do NOT generate TikZ or PGFPlots.\n"
                "10. Include the author command: `\\author{Ramakrishna Kompella}` after the `\\usepackage` commands.\n"
